...
**已经是真的**：`sprite-viewer` 装 `@aio/plugin-sprite` + `createCanvas2dStage`
（骨骼真解析、帧真插值、canvas 上真画），`adv-player` 装 `@aio/plugin-adv` +
`createDomStage`（worksheet 真解析、时间轴真推进、对话框真渲染）。骨骼数据是合成的（铁律 9：素材不进这棵树），
...
- 只剩 `model-3d` 一个占位——它唯一的真实现还依赖一个装不上的上游包
  （见 [ACTIVE.md](../../ACTIVE.md) 阻塞项）。换法见
