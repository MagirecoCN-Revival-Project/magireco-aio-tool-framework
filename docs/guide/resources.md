...
| 位 | 取值 |
|---|---|
| universe | **只校验形状**：`/^[a-z][a-z0-9]*$/`。哪些命名空间存在是**数据**，由清单与交叉表说了算 |
| kind | `character` `sprite` `live2d` `model3d` `voice` `scenario` `card` `item` `bgm` `image` |

::: tip universe 不写白名单
解析器不该知道你的作品叫什么。没见过的命名空间照常解析，随后在 `resolve()`
查不到——那正是铁律 2 要的：**查不到返回空，不猜**。写死一张白名单只会让
「加一个数据源」变成「改一次内核」。

形状还是要管：大写、数字开头、带连字符都拒收，否则 `A:` 与 `a:` 是不是同一个
命名空间就没人说得准了。
:::
...
**不同命名空间下的同一个 ID 可能指向完全不同的实体**：命名空间 b 的 `100101`
是角色乙，命名空间 a 的 `100100` 是角色甲——同号不同人。裸数字在系统里流动，
迟早把一个角色的档案配上另一个角色的模型，而且**不报错**，只是显示了错的人。
:::
