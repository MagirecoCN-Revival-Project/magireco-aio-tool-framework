...
::: danger 配额算不过账，这不是优化问题而是架构问题
免费版与个人版配额相同且 **CPU 时间很紧**；专业版比租 VPS 还贵；即用即付
则一次攻击就能把项目打穿。

这条对嵌入面尤其致命：**嵌入 URL 长在别人的页面上，流量不由我们控制。**
一个 wiki 页上了热门，或者有人对着嵌入 URL 打，烧的都是我们的调用与 CPU。
而 fail-closed 意味着配额打满后嵌入面直接 503——安全上对，可用性上是被打下线了。

这条路径上**唯一真正有效的省法是边缘缓存**：命中缓存的请求根本不触发函数。
所以 `AIO_EMBED_CACHE_SECONDS` 同时是两样东西——

- 你的**下架暴露窗口**（缓存住的响应不会再判一次）；
- 你的**配额乘数**。

缺省 0（不缓存）不是推荐值，只是拒绝替你做决定。**部署前必须明确写一个数。**

> **还有一条不改代码的出路**：`@aio/embed` 不知道 EdgeOne 存在，
> `resolveEmbed()` 是个纯函数，边缘那个文件只是薄适配器。把动态面挪到自己的
> VPS（静态仍留在 EdgeOne 免费 CDN），成本立刻变成可预测的固定值。
> 见 `docs/AIO-ARCHITECTURE.md` 待拍板第 4 条候选丁。
:::
