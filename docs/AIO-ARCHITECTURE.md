...
3. **域名与备案。**
4. ✅ **动态面放在哪、下架怎么兜——已拍板（2026-08-22）：接入 Halo。**

   否掉纯 EdgeOne 的是账，不是设计：免费版与个人版配额相同且 CPU 时间很紧，
   而**触顶会殃及同域名下其他所有 EdgeOne 项目**。EdgeOne 保留为 CDN 与资源面。

   选 Halo 的判据（维护者给出）：完善的管理功能、优秀的可拓展性、
   模块化的功能系统、完善的 SEO 等推广功能；且有付费授权与官方支持。
   Node/TS 那一类不推荐的理由不是不熟，是对不上判据——headless 的
   （Strapi / Directus）不提供前台与 SEO，接了等于前台还得自己写；
   Ghost 的扩展是 webhook 而非进程内插件，对不上「模块化」。

   **这个决定让铁律 11 的困境消失**：正常服务器天然有请求期控制，
   且不按调用计费。原来的三条候选（全量换 / 分级 / 改铁律 11）都不必选。

   ### 要移植的面积比看上去小

   | 层 | 去哪 | 要不要重写 |
   |---|---|---|
   | `@aio/kernel` + 6 个插件 | 浏览器 | **不用**。Halo 只是把 HTML 发出来 |
   | `@aio/core` / `resource` / `registry` | 浏览器 | **不用** |
   | `@aio/site`（路由 / SEO / sitemap） | — | **大部分退休**，那本来就是 CMS 的活 |
   | 嵌入面准入 + 下架判定 | Halo 插件 | 要一份 Java 实现 |

   Halo 2 的插件能用 `RouterFunction` 注册自定义路由、有扩展点机制，
   所以 `/embed/{capability}` 与下架拦截都落得进去。

   **顺带一个收获**：铁律 10 变成平台原生的——Halo 自己的插件启停开关
   就是那「一个开关」，边缘半边与浏览器半边天然共用它，
   不必再维护 `providers.generated.json` 那张对照表。

   ### 重复实现的风险已有对策

   本次会话实测过：MediaWiki 那份 PHP ref 预检与 `parseRef` 在 22 条样本里
   分歧 2 条，其中一条放过了路径穿越的形状。所以判据不能只活在 TS 里——
   `packages/embed/src/corpus.ts` 把准入判定做成了**语言中立的数据语料**
   （20 条，同时 emit 成 `corpus.generated.json`）。Java 实现写完拿它对账，
   条条对上才算接进来。参考实现自己也过这份语料。
