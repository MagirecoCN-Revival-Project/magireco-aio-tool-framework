...
3. **域名与备案。**
4. 🔴 **动态面放在哪、下架怎么兜。**
   三条实测把原方案否掉了：规则引擎与重建同速（不是快路）；路由冲突时静态
   优先（函数不触发）；**而这个平台的配额算不过账**——免费版与个人版配额相同
   且 CPU 时间很紧，专业版比租 VPS 还贵，即用即付则一次攻击就能把项目打穿。

   最后这条尤其打嵌入面：**嵌入 URL 长在别人的页面上，流量不由我们控制。**

   | 候选 | 铁律 11 | 成本 | 备注 |
   |---|---|---|---|
   | 甲 全量换 Function | ✅ | ❌ 2,400 页每请求一次调用 | 按上面的配额现实，基本不可行 |
   | 乙 分级 | 部分 | ⚠️ | 只有可能被下架的走 Function。判据要说得清，否则等于随手划线 |
   | 丙 改铁律 11 | ❌ 放弃 | ✅ | 承认做不到，写明暴露窗口。**放弃 🔴 铁律，必须明确拍板** |
   | **丁 动态面挪出去** | ✅ | ✅ 可预测 | 静态仍在 EdgeOne（免费 CDN，它擅长这个），准入与下架跑在自己的 VPS |

   **丁 值得先算一算**，因为它几乎不要改代码：`@aio/embed` 不知道 EdgeOne
   存在，`resolveEmbed()` 是个纯函数，`functions/embed/[capability].js` 只是
   一层薄适配器——换成 Node/Hono 一个路由即可，判据、测试、MediaWiki 侧
   一行都不动。这正是「不自研运行时、只用平台原语」换来的东西。

   代价是多一个要运维的东西，以及静态与动态跨了两个来源（CORS 与
   `frame-ancestors` 要一起理）。

   > 无论选哪条，**缓存秒数（`AIO_EMBED_CACHE_SECONDS`）都要明确写一个值**。
   > 它同时是下架暴露窗口和唯一有效的省配额手段，缺省 0（不缓存）只是
   > 拒绝替人做决定，不是推荐值。
