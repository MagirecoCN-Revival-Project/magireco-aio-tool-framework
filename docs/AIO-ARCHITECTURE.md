...
4. 🔴 **动态面放在哪、下架怎么兜。**
   ...
   | 候选 | 铁律 11 | 成本 | 备注 |
   |---|---|---|---|
   | 甲 全量换 Function | ✅ | ❌ 2,400 页每请求一次调用 | 按上面的配额现实，不可行 |
   | 乙 分级 | 部分 | ⚠️ | 只有可能被下架的走 Function。判据要说得清，否则等于随手划线 |
   | 丙 改铁律 11 | ❌ 放弃 | ✅ | 承认做不到，写明暴露窗口。**放弃 🔴 铁律，必须明确拍板** |

   > **曾经有过一条「把动态面挪到自建 VPS」的候选，已排除。**
   > 配额扣的是**整个套餐**、不是单个项目，所以拆出去并不隔离成本——
   > 静态那一半照样占同一份额度。记在这里是为了别再走一遍：
   > 「分开部署」在这个计费模型下不是一个省钱手段。

   所以现实的选择只在**乙与丙之间**，而两者都要先回答同一个问题：
   **可以接受多长的下架暴露窗口？** 这个数字定了，剩下的都是实现。
