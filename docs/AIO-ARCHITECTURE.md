...
3. **域名与备案。**
4. 🔴 **内容页要不要放弃静态交付，换取真正的下架能力。**
   实测两条：规则引擎与重建走同一条同步线路（生效速度一样，不是快路）；
   路由冲突时静态优先（函数不触发）。合起来意味着**静态导出的页面没有
   任何请求期下架手段**，与铁律 11 直接冲突。

   唯一可行的形状是「那条路径由 Function 出内容 + 每请求现读强一致清单」，
   嵌入面已经这么做了。三条候选：

   - **甲｜全量换**：2,400 个内容页全部改由 Function 出。铁律 11 成立，
     代价是每请求一次函数调用（成本、延迟、冷启动）与静态交付的全部好处。
   - **乙｜分级**：只有「可能被下架」的那类（剧情正文、语音、立绘）走
     Function，纯派生的索引页仍静态。判据要能说清楚，否则等于随手划线。
   - **丙｜改铁律 11**：承认这个平台上做不到，把下架降级为「重建 + purge」
     并写明暴露窗口有多长。**这是放弃一条 🔴 铁律，必须维护者明确拍板。**
