...
11. **🔴 下架不能只靠重建。** EdgeOne KV 是最终一致的（边缘缓存最长 60 秒），
    ...
    构建期排除出产物与 sitemap，请求期兜住缓存副本。
    缺一个都有暴露窗口。详见 `docs/CMS-ON-EDGEONE.md` §二·五。

    > **🔴 这条目前只有嵌入面做到了，内容页做不到**（2026-08-22 实测）。
    > 本条原先写「请求期由**规则引擎**/Function 兜住」，那半句是错的：
    > 规则引擎与重建走同一条同步线路，**生效速度一样，从来就不是快路**。
    > 加上「路由冲突时静态优先、函数不触发」，静态导出的页面**没有任何
    > 请求期下架手段**。
    >
    > 真正的判据是「每请求现读强一致存储」而非「下发配置等它传播」——
    > 后者不管叫重建、purge 还是规则引擎，延迟下限是同一个。唯一可行的
    > 形状是那条路径由 Function 出内容（嵌入面已如此，见
    > `tools/pack-embed-pages.mjs`）。
    >
    > **✅ 已拍板（2026-08-22）：不走纯 EdgeOne，接入 Halo。**
    > 否掉它的是账不是设计——配额触顶会殃及同域名下其他所有 EdgeOne 项目。
    > 正常服务器天然有请求期控制、且不按调用计费，**所以这条铁律的困境
    > 随换宿主一起消失**，不必降级。EdgeOne 保留为 CDN 与资源面。
    > 详见 `docs/AIO-ARCHITECTURE.md` 待拍板第 4 条（已结）。
...
- **提交前跑 `npm run check`**（typecheck + 测试 + 契约守卫 + **三份守卫自测**）。
  改了内核契约或资源层的，说明哪些测试覆盖了它。

  > 自测那三份是 2026-08-22 补进来的。此前 `check` 只跑守卫本身、不跑
  > 守卫的自测，于是「改了一条策略、把某个自测样本的前提抽掉了」这种事
  > **本地全绿、CI 才红**。实测撞过一次：`example-reader` 的 vendor 禁令
  > 解除后，那条以它为样本的坏样本用例当场失真——守卫仍然拦，但拦的理由
  > 变了。判据是通用的：**本地能跑的检查就别只放在 CI 里**。
